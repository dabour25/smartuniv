...
<div id="content-wrapper">
    ...
    <div class="container-fluid">
      ...
      <ol class="breadcrumb">
        ...
        @if(isset($doct))
        <li class="breadcrumb-item active">Edit Doctor : {{$docd->f_name}} {{$docd->m_name}}</li>
        @else
    ...
    <!-- Page Content -->
        ...
        <div class="card-body">
            @if(isset($did))
        <form class="form" action="/admin/editdoc/{{$docd->did}}" method="post">
            @csrf
          ...
          <div class="form-group">
            ...
            <select aria-controls="dataTable" class="custom-select custom-select-sm form-control form-control-sm" name="dep">
              @foreach($deps as $d)
                <option value="{{$d->id}}" {{isset($docd->department)&&$docd->department==$d->id?'selected':''}}>{{$d->dep_name}}</option>
              @endforeach
            </select>
          </div>
          <div class="form-group">
          ...
          <?php 
            $show=false; 
            if(isset($docd->courses)){
              $pre = explode("|", $docd->courses);
            }else{
              $pre=[];
            }
          ?>
          ...
          <a href="/admin/editdoc" class="btn btn-success">Back To Select</a>
        </form>
    @else
    <!-- Edit Table -->
    <div class="form-inline">
      ...
      <script type="text/javascript">
            $('#s').click(function(){
                if($('#dr').val()==''){
                    location.href='/admin/editdoc';
                }else{
                    location.href='/admin/editdoc/search/'+$('#dr').val();
                }
            });
        </script>
      <br>
          ...
          <table class="table table-dark">
            ...
            <tbody>
              ...
              <tr>
                <th>{{$k+1}}</th>
                <td>
                    <a href="/admin/editdoc/{{$d->did}}"> {{$d->f_name}} {{$d->m_name}} </a>
                </td>
